% >> figS8_batch(100); % simulate 100 runs for examples 1 & 2 
% >> figS8(1); % plot figS8
% >> figS8(2); % plot figS9
...
function figS8(ex)
    ...
    noiseLevel=1;
    resultfilename=fullfile('data',sprintf('figS8_[%g,%g,%g]_10min_noise=%g.mat',Ilat,Imod,Itha,noiseLevel));
    fprintf('Loading %s...',resultfilename);
    tmp=load(resultfilename);
    fprintf('done.\n');
 
    %------ plot input conditions ------
    plot_fig5_S1a(tmp.cond([2,10,end]),[figname 'A.png']);
    plot_histogram(tmp.cond([1,3,5,7,end]),...
                   tmp.all_r_alphaEnvHrfbp_simBOLDbp([1,3,5,7,end],:),...
                   tmp.all_r_gammaEnvHrfbp_simBOLDbp([1,3,5,7,end],:),[figname 'B.png']);
    ...
    for i=[1,length(tmp.cond)]
        simulation(input(:,:,1),duration,dt,noiseLevel,...
            ...
            plotOn,rmbase,sprintf('%s%d.png',figname,i));
    end

    plot_19x19correlationMap_diff2(tmp.cond([1,end]),tmp.all_r_rate([1,end],:,:,:),[figname 'D.png']);
end
