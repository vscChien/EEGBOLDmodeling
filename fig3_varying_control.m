% >> for i=1:10,fig3_batch(10);end  % run 100 simulations and save result. (a couple days)
% >> fig3_constant();    % plot fig3A, fig3B
% >> fig3_varying();     % plot fig3C, fig3D, fig3E
% >> figS3(1);           % plot figS3A
% >> figS3(2);           % plot figS3B
%
% a control version of fig3_varying(),
% The controlled r = corr(env i, BOLD j), where i and j mean two independent
% trials.

function fig3_varying_control()
    ...
    % check saved result
    noiseLevel=1;
    resultfilename=fullfile('data',sprintf('fig3_result_10min_noise=%g_control.mat',noiseLevel));    
    if exist(resultfilename,'file')
        ...
        for c=1:nConds
            for run =1:nRuns 
                ...
                r = simulation_control(input(:,:,run),...% input1 for EEG
                                       input(:,:,mod(run,nRuns)+1), ... % input2 for BOLD
                               duration,dt,noiseLevel,...
    ...
    plot_histogram(cond, all_r_alphaEnvHrfbp_simBOLDbp,all_r_gammaEnvHrfbp_simBOLDbp,'fig3D_control.png');

    plotOn=1; rmbase=1;
    for i=1:3
        simulation_control(input(:,:,1),input(:,:,2),duration,dt,noiseLevel,cond(i).Ilat_range,cond(i).Imod_range,cond(i).Itha_range,plotOn,rmbase,sprintf('fig3E_%d_control.png',i));
    end
...
function plot_single_trial(simMEG,simBOLD_bp,alpha_env_hrf_bp,gamma_env_hrf_bp,...
    ...
    if rmbase % remove baseline
        load(fullfile('data','fig3_baseline.mat'),'baseline');
...
%==========================================================================
% a control version for simulation
% input 1 is used to simulate EEG
% input 2 is used to simulate BOLD
% The simulation time doubles, but it is a quick way for coding.
function r=simulation_control(input1,input2,duration,dt,noiseLevel,Ilat_range,Imod_range,Itha_range,plotOn,rmbase,filename)
    ...
    Iext  = Ilat*rescale(input1(:,1),Ilat_range(1),Ilat_range(2))' + ...
            Imod*rescale(input1(:,2),Imod_range(1),Imod_range(2))' + ...
            Itha*rescale(input1(:,3),Itha_range(1),Itha_range(2))'; 
    ...
    [simEEG, ~, x] = sim_eeg_bold(PSP,time,nE,nPV,nSOM,nVIP,nP,C,dt);
     simEEG=sum(simEEG,1);
    %------ simulate BOLD again------
    Iext2  = Ilat*rescale(input2(:,1),Ilat_range(1),Ilat_range(2))' + ...
            Imod*rescale(input2(:,2),Imod_range(1),Imod_range(2))' + ...
            Itha*rescale(input2(:,3),Itha_range(1),Itha_range(2))';
    [~, time, PSP] = model_jr(W, tau, sigmParam, Iext2, noiseLevel, dt);
    [~, simBOLD, ~] = sim_eeg_bold(PSP,time,nE,nPV,nSOM,nVIP,nP,C,dt);
...
function plot_fig3c(cond)
    ...
    height=5.5;
    set(gcf,'units','centimeters','position',[2 2 width height])
    saveas(gcf,fullfile('figures','fig3C.png'));
end